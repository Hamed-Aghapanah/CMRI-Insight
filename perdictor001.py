...
if show_one_model:
    ...
    model.save(model_name1)
    # Weight_model0 = path_models +'\\weight91.h5'
    # Weight_model0 = path_models +'\\weight15.h5'
    Weight_model0 = path_models +'\\weight125.h5'
...
mask_img = resize(mask_img, (im_width, im_height), mode = 'constant', preserve_range = True)
  
# X   = np.zeros((len(image), im_height, im_width, 1), dtype=np.float32)
X   = np.zeros((2, im_height, im_width, 3), dtype=np.float32)
X[1] = x_img/255.0;
X[0] = x_img/255.0;
Y1   = np.zeros((2, im_height, im_width), dtype=np.float32)
Y2   = np.zeros((2, im_height, im_width), dtype=np.float32)
Y3   = np.zeros((2, im_height, im_width), dtype=np.float32)

Y1[1] = mask_img[:,:,0]/255.0;
Y1[0] = mask_img[:,:,0]/255.0;
Y2[1] = mask_img[:,:,0]/255.0;
Y2[0] = mask_img[:,:,0]/255.0;
Y3[1] = mask_img[:,:,0]/255.0;
Y3[0] = mask_img[:,:,0]/255.0;

p1,p2,p3 = model.predict(X , verbose=1)
index=0

plt.figure()
plt.subplot(3,2,1);plt.imshow( p1[index])
plt.subplot(3,2,2);plt.imshow( p2[index])
plt.subplot(3,2,3);plt.imshow( p3[index])

# XX,YY    =  masker (X,0,Y1,Y2,Y3)
XX1,YY1  =  masker (X,0,p1,p2,p3)
