"""   Created on Tue May 23 17:50:09 2023
...
im_height=128
im_width=128
dual_attention_enable_model_N = 'none'
show_one_model='True'
# =============================================================================
...
model.save(_model0)
all_para = model.count_params()


trainable_count = count_params(model.trainable_weights)
non_trainable_count = count_params(model.non_trainable_weights)
# print('Total Parameters new  model        =   ' + mil(all_para))
...
import os
import numpy as np
# model_name='final_model n DAe 1time12_07_22.h5'
model_path_all=os.getcwd()+'\\loading_segmentation_model2'
f = os.listdir(model_path_all)
ff=[]

# from model_CardSegNet import CardSegNet
# model =  CardSegNet ()
...
w_p_weight=[]
for i in f:
    if  i[-3:]=='.h5' and   not i[0]=='w' :
            ff.append(i)
            model_path=model_path_all+'\\'+i
            # print(i)
            w_p_models.append(model_path)
            # print(model_path+'  ok')
    ...
    if  i[-3:]=='.h5' and   i[0]=='w' :
        ...
        # except:s=1#print(model_path+'  NO') 
...
image_no=30        
X   = np.zeros((image_no, im_height, im_width, 1), dtype=np.float32)
X3   = np.zeros((image_no, im_height, im_width, 3), dtype=np.float32)
Y1  = np.zeros((image_no, im_height, im_width, 1), dtype=np.float32)
Y2  = np.zeros((image_no, im_height, im_width, 1), dtype=np.float32)
Y3  = np.zeros((image_no, im_height, im_width, 1), dtype=np.float32)        

path1=os.getcwd()+'\\dataset\\masks2'
files1  = os.listdir(path1)
path0=os.getcwd()+'\\dataset\\images2'
for index1 in range(np.shape(X)[0] ):
    a=index1
    ...
    i1=np.where (masks==64);i2=np.where (masks==128);i3=np.where (masks==191)
    ...
    gray_image = cv2.cvtColor(x_img, cv2.COLOR_BGR2GRAY)        
    X[index1,:,:,0] = gray_image/255.0; 
    X3[index1,:,:,:] = x_img/255.0; 
    
    Y1[index1] = masks1/255.0
    Y2[index1] = masks2/255.0;
    Y3[index1] = masks3/255.0
...
#                n_filters = 2, dropout = 0.1, batchnorm = True) 
# # opt = Adam(learning_rate=INIT_LR, decay=INIT_LR/epochs_no)
# model.compile(optimizer = 'Adam',loss=losses1,# model.compile(optimizer = opt,loss=losses1,
#                metrics = metrics_all )

# all_para = model.count_params()
# trainable_count = count_params(model.trainable_weights)
# non_trainable_count = count_params(model.non_trainable_weights)
# print('Total Parameters new  model  1        =   ' + mil(all_para))
# print('Total non_trainable_count P  model  1 =  -' + mil(non_trainable_count))
# print('Total trainable_count P model  1      =   ' + mil(trainable_count))
# # os.startfile( model_name1)


# print( 'len(model.layers) ='+str(len(model.layers)))
# ssss
# for w_p1 in w_p_models:
for w_p2 in w_p_weight:
    if con1:
        # model_path=w_p1
        wmodel_path=w_p2
        cnt=cnt+1;
        # for dual_attention_enable_model_N in ['v','c','s','vs','cs','sv','svc']:
        for dual_attention_enable_model_N in ['svc']:
            # for name_back_bone in ['resnet','none']:
            for name_back_bone in ['resnet' ]:
                model=BBVDAMLF_model(input_img,
                               bb_name=name_back_bone, 
                               dual_attention_enable_model=dual_attention_enable_model_N ,
                               n_filters = 2, dropout = 0.1, batchnorm = True) 
                ...
                print('Total trainable_count P model       1  =   ' + mil(trainable_count))
                print(' len(model.layers) ='+str( len(model.layers)))
                print(dual_attention_enable_model_N , name_back_bone )
                # model= load_model(model_path)
                # all_para = model.count_params()
                # trainable_count = count_params(model.trainable_weights)
                # model= load_model(model_path)
                print('+')
                model.load_weights(wmodel_path)
                print('++')
                
                # sss
                print(wmodel_path)
                p1,p2,p3 = model.predict(X3 , verbose=1)
                if just_one==1:
                    model.save('temp.h5')
                    os.startfile('temp.h5')
                    os.startfile('res')
                for index1 in range(30 ):     
                    plt.figure()
                    plt.subplot(131);plt.imshow(X[index1],cmap='gray')
                    plt.subplot(263);plt.imshow(Y1[index1],cmap='gray')
                    plt.subplot(264);plt.imshow(Y2[index1],cmap='gray')
                    plt.subplot(265);plt.imshow(Y3[index1],cmap='gray')
                    Y=np.zeros((im_height ,im_width,3))
                    Y[:,:,0]=Y1[index1,:,:,0];Y[:,:,1]=Y2[index1,:,:,0];Y[:,:,2]=Y3[index1,:,:,0]
                    plt.subplot(266);plt.imshow(Y ,cmap='gray')
                    
                    plt.subplot(2,6,9);plt.imshow(p1[index1],cmap='gray')
                    plt.subplot(2,6,10);plt.imshow(p2[index1],cmap='gray')
                    plt.subplot(2,6,11);plt.imshow(p3[index1],cmap='gray')
                    Z=np.zeros((im_height ,im_width,3))
                    Z[:,:,0]=p1[index1,:,:,0];Z[:,:,1]=p2[index1,:,:,0];Z[:,:,2]=p3[index1,:,:,0]
                    plt.subplot(2,6,12);plt.imshow(Z ,cmap='gray')
                    plt.savefig('res//res sample='+str(index1 )+'___ cnt'+ str(cnt)+'.png')
                    plt.close('all')
                    cnt1=cnt1+1
                    print(cnt,cnt1)
                    if cnt1== 15:
