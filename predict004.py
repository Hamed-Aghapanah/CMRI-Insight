...
if finding==0:
    ...
    for i in indexes:
        ...
        def max_segman (image):
            ...
            # cv2.waitKey(300)
            return output
        # try:p01[0,:,:,0]=max_segman(p01[0,:,:,0] )
        # except:s=1
        # try:p02[0,:,:,0]=max_segman(p02[0,:,:,0] )
        # except:s=1
        # try:p03[0,:,:,0]=max_segman(p03[0,:,:,0] )    
        # except:s=1
        # plt.figure();
        # plt.imshow(p01)
        # plt.figure();
        ...
        ddd='RV='+str(d_r) +' ,LV='+str(d_l) +' , Myo= '+str(d_m) +' , B= '+str(d_b) +' , overal'+str(d_overal) 
        ...
        fig.savefig('res_dice2\\Grad_'+str(cnt_dice)+'.png' )
        plt.close(1000)
        if just_one==1:
            just_one=0
...
import numpy as np
data1 = np.load('indexxx2.npz', allow_pickle = True)
DICE_ALL = data1['DICE_ALL']
